...
app.get(
  "/cbt/events/:id/",
  requireAuthorization,
  cbt.handleCBTEventMultipleGet(db)
);
...
app.get(
  "/copingskills/:id",
  requireAuthorization,
  copingSkills.handleCopingSkillsGet(db)
);
// delete coping skill from a users list.
app.delete(
  "/copingskills/:id/:skill_id",
  requireAuthorization,
  copingSkills.handleCopingSkillsDelete(db)
);
// Insert new coping skill to users list. (And share if shared).
app.post(
  "/copingskills/:id",
  requireAuthorization,
  copingSkills.handleCopingSkillsPost(db)
);
// Update existing skill on users list.
app.put(
  "/copingskills/:id/:skill_id",
  requireAuthorization,
  copingSkills.handleCopingSkillsPut(db)
);
// Share a user's coping skill.
app.put(
  "/copingskills/share/:id/:skill_id",
  requireAuthorization,
  copingSkills.handleCopingSkillsSharePut(db)
);

// Shared coping skills.
// Get shared skills by new, top, or rand.
app.get(
  "/copingskills/shared/:id/:type",
  copingSkills.handleCopingSkillsSharedGet(db)
);
// Add a shared skill to users own list.
app.post(
  "/copingskills/shared/:id/:skill_id",
  requireAuthorization,
  copingSkills.handleCopingSkillsSharedPost(db)
);
...
app.get(
  "/dm/:id/:date",
  requireAuthorization,
  dm.handleDailyMaintenanceGet(db)
);
// Get tasks when clicking next/previous (:change) date from current :date
app.get(
  "/dm/:id/:date/:change",
  requireAuthorization,
  dm.handleDailyMaintenanceGetDateChange(db)
);
// Delete a user's task.
app.delete(
  "/dm/:id/:task_id",
  requireAuthorization,
  dm.handleDailyMaintenanceDelete(db)
);
...
app.put(
  "/dm/:id/:task_id/:completed",
  requireAuthorization,
  dm.handleDailyMaintenancePutComplete(db)
);
...
app.get(
  "/history/dm/:id/",
  requireAuthorization,
  history.handleHistoryGetDM(db)
);
// PHQ9 score by date
app.get(
  "/history/phq9/:id",
  requireAuthorization,
  history.handleHistoryGetPHQ9(db)
);
// CBT Thinking styles
app.get(
  "/history/cbt/thinkingstyles/:id",
  requireAuthorization,
  history.handleHistoryGetCBTThinkingStyles(db)
);
// CBT Before/After Ratings
app.get(
  "/history/cbt/ratings/:id",
  requireAuthorization,
  history.handleHistoryGetCBTRatings(db)
);
